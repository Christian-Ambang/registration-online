$(document).ready(function () {
  ...
  $(".niveauEntrainement,.naissanceEnfant").on("change", function () {
    ...
    var dateVal = $(this).parent().find(".naissanceEnfant").val();
    var niveauVal = $(this).parent().find(".niveauEntrainement").val();

    if (dateNaissance(dateVal) == true && niveauVal != "") {
      for (var i = 0; nbCoursSemaine.length > i; i++) {
        if (
          ageComprisEntre(dateVal, nbCoursSemaine[i][2]) == true &&
          niveauVal == nbCoursSemaine[i][1]
        ) {
          var optionSearch = optionAllDetach[indexNiveau].filter(
            "option[value=" + nbCoursSemaine[i][0] + "]"
          );
    ...
    if (dateNaissance(dateValNiveau) == true) {
      for (var i = 0; nbCoursSemaine.length > i; i++) {
        if (ageComprisEntre(dateValNiveau, nbCoursSemaine[i][2])) {
          var optionSearchNiveau = optionAllDetachNiveau[indexNiveau].filter(
            "option[value=" + nbCoursSemaine[i][1] + "]"
          );
